#!/usr/bin/env python

import sys
import json
import gzip
import os.path

if len(sys.argv) < 4:
    print(f"usage: {sys.argv[0]} <output-directory> <prefix> <infile> [<infile2> ...]", file=sys.stderr)
    print("input files are expected to be in jsonl.gz format", file=sys.stderr)
    sys.exit(1)

outdir = sys.argv[1]
prefix = sys.argv[2]
infiles = sys.argv[3:]

# determine output file paths within output directory
en_fn = os.path.join(outdir, f"{prefix}-en.tsv.gz")
de_fn = os.path.join(outdir, f"{prefix}-de.tsv.gz")

def clean_text(text):
    return ' '.join(text.strip().split())


def write_tsv(f, text, uris):
    uris_fmt = ' '.join([f"<{uri}>" for uri in uris])
    print(f"{text}\t{uris_fmt}", file=f)


def process_file(in_f, en_f, de_f):
    for line in in_f:
        rec = json.loads(line)
        en_text = clean_text(rec['text_en'].replace('\n\n', ' ¤ '))
        de_text = clean_text(rec['text_de'].replace('\n\n', ' ¤ '))
        write_tsv(en_f, en_text, rec['subjects'])
        write_tsv(de_f, de_text, rec['subjects'])


with gzip.open(en_fn, "wt") as en_f, \
     gzip.open(de_fn, "wt") as de_f:
    for in_fn in infiles:
        with gzip.open(in_fn) as in_f:
            process_file(in_f, en_f, de_f)
